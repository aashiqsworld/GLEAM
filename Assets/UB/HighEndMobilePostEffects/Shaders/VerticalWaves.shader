...
Shader "UB/PostEffects/VerticalWaves" {
    Properties {
	...
	Subshader{
		...
		Pass{
			...
			half4 frag(vertexOutput i) : SV_Target
			{
			    ...
			    float z = floor((x/_SizeX) + 0.5);
			    
			    float y2 = y + (sin(z + (_Time.x * _Speed)) * _SizeY);
			    
			    float2 uv2 = float2(x/_ScreenParams.x, y2/_ScreenParams.y);
